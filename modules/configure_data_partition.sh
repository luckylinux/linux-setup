...
then
        ...
        echo "${disk1}_data_crypt" UUID=$(blkid -s UUID -o value ${device1}-part${data_num}) none \
        luks,discard,initramfs > "${destination}/etc/crypttab"

        if [ $numdisks -eq 2 ]
        then
                echo "${disk2}_data_crypt" UUID=$(blkid -s UUID -o value ${device2}-part${data_num}) none \
                 luks,discard,initramfs >> "${destination}/etc/crypttab"
        fi
fi
...
then
        ...
        then
                ...
                #mdadm --detail --scan | grep "/dev/${mdadm_data_device}" >> /etc/mdadm/mdadm.conf
...
# Define mdadm Device
mdadm_device="/dev/${mdadm_data_device}"

# List Member Devices
member_devices=()
member_devices+=( "${devices[0]}-part${data_num}" )
member_devices+=( "${devices[1]}-part${data_num}" )
EOF
        ...
        then
                ...
                UUID=$(blkid -s UUID -o value $device1-part${data_num})
